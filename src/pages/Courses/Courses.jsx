...
import { FaBookmark, FaBookReader, FaClock, FaEject, FaStar } from "react-icons/fa";
import { Link } from "react-router-dom";
import Swal from "sweetalert2";
import { AuthContext } from "../../Contexts/AuthProvider";
...
const Courses = () => {
  ...
  useEffect(() => {
    ...
    let filtered = data.filter(course => {
      return (
        (course.category.includes(selectedCategory) || selectedCategory === '') &&
        (course.instructor.instructor_name.toLowerCase().includes(selectedInstructor.toLowerCase()) || selectedInstructor === '') &&
  ...
  const uniqueInstructors = [...new Set(data.map(course => course.instructor.instructor_name))];
  ...
  return (
    <div>
      ...
      <div className="bg-gray-100 p-4 space-y-4">
        ...
        <label htmlFor="ratingSlider" className="text-lg font-semibold">Filter by Rating:</label>
        <input
          type="range"
          id="ratingSlider"
          min={0}
